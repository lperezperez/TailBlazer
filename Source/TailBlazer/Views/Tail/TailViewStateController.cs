namespace TailBlazer.Views.Tail
{
    ...
    public class TailViewStateController : IDisposable
    {
        ...
        public TailViewStateController([NotNull] TailViewModel tailView, IStateBucketService stateBucketService, ISchedulerProvider schedulerProvider, ITailViewStateRestorer tailViewStateRestorer, ILogger logger, bool loadDefaults)
        {
            ...
            //Load state
            if (loadDefaults)
                stateBucketService.Lookup(type, tailView.Name).IfHasValue
                    (
                     tailviewstate =>
                         {
            ...
            var selectedChanged = tailView.SearchCollection.WhenValueChanged(sc => sc.Selected, false).Where(vm => vm != null).Select(vm => vm.Text);
            var metaChanged = tailView1.SearchMetadataCollection.Metadata.Connect().ToCollection().Select(metaData => metaData.ToArray());
            var writer = selectedChanged.CombineLatest(metaChanged, (selected, metadata) => new { selected, metadata }).Where(_ => !loadingSettings).Throttle(TimeSpan.FromMilliseconds(250)).Select(x => converter.Convert(tailView1.Name, x.selected, x.metadata)).Subscribe(state => { stateBucketService.Write(type, tailView.Name, state); });
            this._cleanUp = new CompositeDisposable(writer);
        }
        #endregion
        #region Methods
