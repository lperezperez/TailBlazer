namespace TailBlazer.Views.Recent
{
    ...
    public class RecentSearchToStateConverter : IConverter<RecentSearch[]>
    {
        ...
        public State Convert(RecentSearch[] files)
        {
            ...
            var fileNodeArray = files.Select(f => new XElement(Structure.File, new XAttribute(Structure.Name, f.Text), new XAttribute(Structure.Date, f.Timestamp)));
            fileNodeArray.ForEach(root.Add);
            var doc = new XDocument(root);
            return new State(2, doc.ToString());
        }
        public RecentSearch[] GetDefaultValue() => new RecentSearch[0];
        #endregion
        #region Classes
