    ...
    public class StartFromFixture
    {
        ...
        public void EmptyForEmptyFile()
        {
            ...
            using (var file = new TestFile())
                using (file.Info.WatchFile(scheduler: scheduler).ScanFrom(0, scheduler: scheduler).Subscribe(x => result = x))
                {
        ...
        public void SkipFirstLine()
        {
            ...
            var scheduler = new TestScheduler();
            using (var file = new TestFile())
                using (file.Info.WatchFile(scheduler: scheduler).ScanFrom(startPosition, scheduler: scheduler).Subscribe(x => result = x))
                {
        ...
        public void StartAfterEndOfFileShouldReturnNothing()
        {
            ...
            var scheduler = new TestScheduler();
            using (var file = new TestFile())
                using (file.Info.WatchFile(scheduler: scheduler).ScanFrom(10000, scheduler: scheduler).Subscribe(x => result = x))
                {
                    file.Append("A");
                    file.Append("B");
                    file.Append("C");
        ...
        public void StartFromBegining()
        {
            ...
            var length = (firstLine + Environment.NewLine + secondLine + Environment.NewLine + thirdLine + Environment.NewLine).Length;
            FileNotification result = null;
            var scheduler = new TestScheduler();
            using (var file = new TestFile())
                using (file.Info.WatchFile(scheduler: scheduler).ScanFrom(0, scheduler: scheduler).Subscribe(x => result = x))
        ...
        public void StartFromSecondLine()
        {
            ...
            var startPosition = (firstLine + Environment.NewLine).Length;
            var length = (firstLine + Environment.NewLine + secondLine + Environment.NewLine + thirdLine + Environment.NewLine + fourthLine + Environment.NewLine).Length - startPosition;
            FileNotification result = null;
            var scheduler = new TestScheduler();
            using (var file = new TestFile())
                using (file.Info.WatchFile(scheduler: scheduler).ScanFrom(startPosition, scheduler: scheduler).Subscribe(x => result = x))
