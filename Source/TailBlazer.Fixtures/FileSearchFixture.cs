    ...
    public class FileSearchFixture
    {
        ...
        public void CanReadLinesBack()
        {
            ...
            using (var file = new TestFile())
            {
                ...
                using (file.Info.WatchFile(pulse).Search(str => str.Contains("9"), scheduler).Subscribe(x => fileSearchResult = x))
                {
                    pulse.Once();
        ...
        public void CanReadLinesBackInLargeFile()
        {
            ...
            using (var file = new TestFile())
            {
                ...
                using (file.Info.WatchFile(pulse).Search(str => str.Contains("9"), scheduler).Select(result => file.Info.ReadLinesByPosition(result.Matches).ToArray()).Subscribe(x => lines = x))
                {
                    pulse.Once();
                    // fileSearchResult.Matches.Length.Should().Be(19);
                    file.Append(new[] { "9", "20" });
        ...
        public void CreateFileLater()
        {
            ...
            using (var file = new TestFile())
            {
                file.Delete();
                FileSearchResult fileSearchResult = null;
                using (file.Info.WatchFile(pulse).Search(str => str.Contains("9")).Subscribe(x => fileSearchResult = x))
                {
        ...
        public void EmptyFile()
        {
            ...
            {
                FileSearchResult fileSearchResult = null;
                using (file.Info.Search(str => str.Contains("9"), scheduler).Subscribe(x => fileSearchResult = x))
                {
        ...
        public void InitiallyEmptyThenLinesAdded()
        {
            ...
            {
                FileSearchResult fileSearchResult = null;
                using (file.Info.WatchFile(scheduler: scheduler).Search(str => str.Contains("9")).Subscribe(x => fileSearchResult = x))
                {
