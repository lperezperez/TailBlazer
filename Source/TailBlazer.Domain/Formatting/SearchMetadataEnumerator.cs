namespace TailBlazer.Domain.Formatting
{
    ...
    {
        #region Constants
        private const RegexOptions CaseInsensitiveOptions = RegexOptions.IgnorePatternWhitespace | RegexOptions.Compiled | RegexOptions.IgnoreCase;
        private const RegexOptions CaseSensitiveOptions = RegexOptions.IgnorePatternWhitespace | RegexOptions.Compiled;
        #endregion
        #region Fields
        ...
        private static IEnumerable<MatchedString> Yield(string input, SearchMetadata tomatch)
        {
            ...
            if (tomatch.UseRegex)
            {
                ...
                var childMatches = new MatchedStringEnumerator2(input, matches.Cast<Match>().Select(match => match.Value).ToArray(), tomatch);
                foreach (var item in childMatches)
                    yield return item;
            }
            else
            {
                foreach (var item in SearchMetadataEnumerator.Yield2(input, tomatch))
                    if (!item.IsMatch)
        ...
        public IEnumerator<MatchedString> GetEnumerator()
        {
            ...
            for (var i = 0; i < strings.Length; i++)
            {
                ...
                if (i == 0)
                    matches = SearchMetadataEnumerator.Yield(this._input, searchMetadata).ToArray();
                else
                    matches = matches.SelectMany(ms => ms.IsMatch ? new[] { ms } : SearchMetadataEnumerator.Yield(ms.Part, searchMetadata)).ToArray();
            }
