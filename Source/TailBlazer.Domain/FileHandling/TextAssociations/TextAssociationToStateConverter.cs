namespace TailBlazer.Domain.FileHandling.TextAssociations
{
    ...
    public class TextAssociationToStateConverter : IConverter<TextAssociation[]>
    {
        ...
        public TextAssociation[] Convert(State state)
        {
            ...
            var files = root.Elements(Structure.TextAssociation).Select
                (
                 element =>
                     {
        ...
        public State Convert(TextAssociation[] items)
        {
            ...
            var itemsNode = items.Select(f => new XElement(Structure.TextAssociation, new XElement(Structure.Text, f.Text), new XAttribute(Structure.UseRegEx, f.UseRegEx), new XAttribute(Structure.IgnoreCase, f.IgnoreCase), new XAttribute(Structure.Swatch, f.Swatch), new XAttribute(Structure.Hue, f.Hue), new XAttribute(Structure.Icon, f.Icon), new XAttribute(Structure.Date, f.DateTime)));
            itemsNode.ForEach(root.Add);
            var doc = new XDocument(root);
            return new State(1, doc.ToString());
        }
        public TextAssociation[] GetDefaultValue() => new TextAssociation[0];
        #endregion
        #region Classes
